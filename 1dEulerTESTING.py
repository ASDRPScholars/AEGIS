...
Nx_exact = 4096   # Number of grid points in the "exact" solution
L = 1.0      # Domain length
dx = L / (Nx - 1)  # Spatial step
dx_exact = L / (Nx_exact - 1)  # Spatial step for the "exact" solution
CFL = 0.4   # Courant number, going with the lit

# Spatial grid
x = np.linspace(0, L, Nx)
x_exact = np.linspace(0, L, Nx_exact)
...
# Initialize the exact solution arrays
rho_exact = np.zeros(Nx_exact)
p_exact = np.zeros(Nx_exact)
u_exact = np.zeros(Nx_exact)
E_exact = np.zeros(Nx_exact)
mom_exact = np.zeros(Nx_exact)
...
line_rho_blend = line_rho_WENO = line_rho_exact = line_rho_first = None

line_u_blend = line_u_WENO = line_u_exact = line_u_first = None

line_p_blend = line_p_WENO = line_p_exact = line_p_first = None

# Select initial conditions
IC_type = "lax_tube"  # Options: "sod", "shu-osher", "lax_tube", "woodward", "Einfeldt", "Gauss"
# Note: Woodward-Colella (woodward) + Einfeldt are currently work in progress; don't use!
...
if IC_type == "sod":
    ...
    # Same set up for the exact values
    rho_exact[:] = 1.0
    rho_exact[x_exact >= 0.5] = 0.125

    p_exact[:] = 1.0
    p_exact[x_exact >= 0.5] = 0.1
    ...
    # Same set up for the exact values
    rho_r_exact = 1.0 + 0.2 * np.sin(50 * x_exact)
    p_r_exact = np.ones(Nx_exact)
    u_r_exact = np.zeros(Nx_exact)

    rho_exact = np.where(x_exact < shock_pos, rho_l, rho_r_exact)
    p_exact = np.where(x_exact < shock_pos, p_l, p_r_exact)
    u_exact = np.where(x_exact < shock_pos, u_l, u_r_exact)

    bc = "normal"

    t_final = 0.18 # Final time
    # t_final = 0.085 # Modified for quick testing

elif IC_type == "lax_tube":
    # The Lax Shock Tube problem
    # I think it tests the same thing as Sod, but just more validation
    ...
    # Same set up for the exact values
    rho_exact = np.where(x_exact < shock_pos, rho_l, rho_r)
    p_exact = np.where(x_exact < shock_pos, p_l, p_r)
    u_exact = np.where(x_exact < shock_pos, u_l, u_r)
    ...
    # This is the Woodward-Colella test problem; just writing it in case it
    # ends up being needed for result validation
    ...
    rho_exact[:] = 1.0
    u_exact[:] = 0.0
    p_exact = np.where(x_exact > shock_pos1, p_mid, p_l)
    p_exact[x_exact > shock_pos2] = p_r
    ...
    # This is literally just for the convergence study on a smooth problem
    # Don't bother running for shock testing

    rho = 1.0 + 0.1*np.exp(-((x - 0.5)**2)/(2*0.1**2))
    u = 1.0
    p = 1.0

    rho_exact = 1.0 + 0.1*np.exp(-((x_exact - 1.0)**2)/(2*0.1**2))
    u_exact = 1.0
    p_exact = 1.0
...
E_exact = p_exact / (gamma - 1) + 0.5 * rho_exact * u_exact**2
mom_exact = rho_exact * u_exact
...
while t < t_final:
    ...
    c_exact = np.sqrt(gamma * p_exact / rho_exact)
    ...
    # Just gonna check how many times the exact solution will run in one time step for the other methods

    subcycles = int(np.ceil(dt / dt_exact))

    # Making it so that dt_exact is going running a WHOLE NUMBER of times for each dt
    dt_exact = dt / subcycles
    ...
    if np.any(rho_exact <= 0) or np.any(p_exact <= 0):
      print("[EXACT] Unphysical starting state at time n", t, step)
      break
    ...
    # Updating the reference/"exact" solution in multiple timesteps
    # Takes as many timesteps as necessary to "catch-up" with the other methods

    for _ in range(subcycles):
      rho_exact, mom_exact, E_exact = WENO_solution(rho_exact, mom_exact, E_exact, dt_exact, dx_exact)
    ...
    if step%50 == 0:
        ...
        rho_plot_exact, u_plot_exact, p_plot_exact = cons_to_prim(rho_exact, mom_exact, E_exact)
        ...
        if createPlot:
            ...
            line_rho_WENO,  = ax1.plot(x, np.zeros_like(x), color='b', label="WENO5-JS Density")
            line_rho_exact, = ax1.plot(x_exact, np.zeros_like(x_exact), color='g', label="Exact Density")
            line_rho_first, = ax1.plot(x, np.zeros_like(x), color='y', label="First Order Density")
            ax1.set_xlabel("x")
            ...
            line_u_WENO,  = ax2.plot(x, np.zeros_like(x), color='b', label="WENO5-JS Velocity")
            line_u_exact, = ax2.plot(x_exact, np.zeros_like(x_exact), color='g', label="Exact Velocity")
            line_u_first, = ax2.plot(x, np.zeros_like(x), color='y', label="First Order Velocity")
            ax2.set_xlabel("x")
            ...
            line_p_WENO,  = ax3.plot(x, np.zeros_like(x), color='b', label="WENO5-JS Pressure")
            line_p_exact, = ax3.plot(x_exact, np.zeros_like(x_exact), color='g', label="Exact Pressure")
            line_p_first, = ax3.plot(x, np.zeros_like(x), color='y', label="First Order Pressure")
            ax3.set_xlabel("x")
            ...
            # Used just for debugging the code

            line_sensor, = ax4.plot(
                x[2:-2],
                np.zeros_like(x[2:-2]),
                color='k',
                label='Shock Sensor'
            ...
            title_sensor = ax4.set_title("Shock Sensor (t=0.000)")
            ax4.grid()
            ax4.legend()
            '''
            ...
            line_rho_WENO.set_ydata(rho_plot_WENO)
            line_rho_exact.set_ydata(rho_plot_exact)
            line_rho_first.set_ydata(rho_plot_first)
            title_rho.set_text(f"Density Profile (t={t:.3f})")

            '''

            line_u_blend.set_ydata(u_plot_blend)
            line_u_WENO.set_ydata(u_plot_WENO)
            line_u_exact.set_ydata(u_plot_exact)
            line_u_first.set_ydata(u_plot_first)
            title_u.set_text(f"Velocity Profile (t={t:.3f})")

            line_p_blend.set_ydata(p_plot_blend)
            line_p_WENO.set_ydata(p_plot_WENO)
            line_p_exact.set_ydata(p_plot_exact)
            line_p_first.set_ydata(p_plot_first)
            title_p.set_text(f"Pressure Profile (t={t:.3f})")


            shockArray = shock_Detector(p_plot_blend)

            line_sensor.set_ydata(shockArray)
            title_sensor.set_text(f"Shock Sensor (t={t:.3f})")
...
rho_plot_WENO, u_plot_WENO, p_plot_WENO = cons_to_prim(
    rho_WENO, mom_WENO, E_WENO
)
rho_plot_exact, u_plot_exact, p_plot_exact = cons_to_prim(
    rho_exact, mom_exact, E_exact
)
rho_plot_first, u_plot_first, p_plot_first = cons_to_prim(
    rho_first, mom_first, E_first
)
...
# Exclude three boundary-buffer points from each side
physical = slice(3, -3)

# Interpolate the interior reference solution onto the interior coarse grid
rho_ref = np.interp(x[physical], x_exact[physical], rho_plot_exact[physical])

rho_WENO_error_L1 = np.mean(np.abs(rho_ref - rho_plot_WENO[physical]))
rho_blend_error_L1 = np.mean(np.abs(rho_ref - rho_plot_blend[physical]))
rho_first_error_L1 = np.mean(np.abs(rho_ref - rho_plot_first[physical]))

rho_WENO_error_L2 = np.sqrt(np.sum((rho_ref-rho_plot_WENO[physical])**2)/len(rho_plot_WENO[physical]))
rho_blend_error_L2 = np.sqrt(np.sum((rho_ref-rho_plot_blend[physical])**2)/len(rho_plot_blend[physical]))
rho_first_error_L2 = np.sqrt(np.sum((rho_ref-rho_plot_first[physical])**2)/len(rho_plot_first[physical]))

# Easiest fixes for a visual clutter
print()

print(f"L1 WENO Error: {rho_WENO_error_L1}")
print(f"L1 Blended Error: {rho_blend_error_L1}")
print(f"L1 First Error: {rho_first_error_L1}")

print()

print(f"L2 WENO Error: {rho_WENO_error_L2}")
print(f"L2 Blended Error: {rho_blend_error_L2}")
print(f"L2 First Error: {rho_first_error_L2}")
