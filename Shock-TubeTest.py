...
# Note:
# Woodward-Colella (woodward) will crash WENO5-jS, probably due to it being
# component-wise and not the characteristic form
...
while t < t_final:
    ...
    if step%100 == 0:
        ...
        if createPlot:
            ...
            line_rho_exact, = ax1.plot(x_exact, np.zeros_like(x_exact), color='g', label="Reference Density")
            line_rho_first, = ax1.plot(x, np.zeros_like(x), color='y', label="FO-LF Density")
            ax1.set_xlabel("x")
            ...
            line_u_exact, = ax2.plot(x_exact, np.zeros_like(x_exact), color='g', label="Reference Velocity")
            line_u_first, = ax2.plot(x, np.zeros_like(x), color='y', label="FO-LF Velocity")
            ax2.set_xlabel("x")
            ...
            line_p_exact, = ax3.plot(x_exact, np.zeros_like(x_exact), color='g', label="Reference Pressure")
            line_p_first, = ax3.plot(x, np.zeros_like(x), color='y', label="FO-LF Pressure")
            ax3.set_xlabel("x")
